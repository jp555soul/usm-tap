...
class MobileAuthRepository implements AuthRepository {
  ...
  MobileAuthRepository({required FlutterAppAuth appAuth}) : _appAuth = appAuth;
  ...
  Future<Either<Failure, UserEntity>> login() async {
    try {
      ...
      if (result == null) return Left(const AuthFailure('Login cancelled'));
...
AuthRepository createPlatformAuthRepository(dynamic appAuth) {
  return MobileAuthRepository(appAuth: appAuth as FlutterAppAuth);
}
